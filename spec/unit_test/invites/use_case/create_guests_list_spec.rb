describe Invites::UseCase::CreateGuestsList do
  let(:file_url) { 'https://s3.amazonaws.com/intercom-take-home-test/customers.txt' }
  subject { Invites::UseCase::CreateGuestsList.new(customer_file: file_url, presenter: presenter) }
  
  # That's what's actually in the file, jsut to show off it easily to reviewers
...
{"latitude": "52.833502", "user_id": 25, "name": "David Behan", "longitude": "-8.522366"}
  ...
  before do
    stub_request(:get, file_url).to_return(body: blob_data)
  end

  context 'when presenting to text format' do
    let(:presenter) { Invites::Presenter::InvitesAsTxt.new(
      attributes: [:name, :user_id],
      sort_key: :user_id
    )}

    it 'creates a guest lists from customers closer than or equal to 100km' do
      expect(subject.call(in_range_of_km: 100))
